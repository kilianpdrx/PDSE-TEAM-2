...
const int dirPin_right = 6, stepPin_right = 4;
const int ena_right = 7;
...
void setup() {
  ...
  Timer1.initialize(500);  //trigger ever microsecond
...
void loop() {
  ...
  //  Read from Raspberry
  if (millis() - lastRaspTime >= 100) {  // Read every 100 ms
    lastRaspTime = millis();
  ...
  if (!initialTestDone) {  //Initial move
    ...
    Serial.print("Initial test complete. Switching to normal operation.");
  }
